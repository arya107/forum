<template>
  <transition name="slide-fade">
    <div class="thread_post_notification" @click="$emit('goToPost')">
      <span
        class="thread_post_notification__close"
        @click.stop="$emit('close')"
      ></span>
      <div class="thread_post_notification__header_bar">
        <span class="thread_post_notification__username">{{
          post.username
        }}</span>
        replied &nbsp;&middot;&nbsp; click to view
      </div>
      <div class="thread_post_notification__content">
...
.thread_post_notification {
  ...
  box-shadow: 0 0 0.5rem 1px rgba(175, 175, 175, 0.3),
    0 0.2rem 0.3rem 0px rgba(175, 175, 175, 0.15);
