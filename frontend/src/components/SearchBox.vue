<template>
  <div class="search_box" ref="root">
    <div class="search_box__input" tabindex="0" @keydown.enter="goToSearch">
      <input
      ...
      />
      <button class="search_box__input__button" @click="goToSearch">
      ...
      <div class="search_box__results__container" ref="results">
        <template v-if="threads.length">
          ...
          <div
            ...
            :class="{
              'search_box__results--highlight':
                highlightIndex === getHighlightIndex('threads header')
            }"
            ref="threads header"
            @mouseover="highlightIndex = getHighlightIndex('threads header')"
            @click="goToSearch"
          >
            <div class="search_box__results__icon">
          ...
          <div
            ...
            :class="{
              'search_box__results--highlight':
                highlightIndex === getHighlightIndex('threads', index)
            }"
            ...
            @click="goToSearch"
          >
            <div class="search_box__results__title">
              {{ thread.name | truncate(50) }}
            </div>
            <div class="search_box__results__content">
              {{ thread.Posts[0].content | stripTags | truncate(75) }}
            </div>
          </div>
        </template>
        ...
        <template v-if="users.length">
          <div
            class="search_box__results__header search_box__results__header--divider"
          >
            Users
          </div>
          <div
            ...
            :class="{
              'search_box__results--highlight':
                highlightIndex === getHighlightIndex('users header')
            }"
            ref="users header"
          ...
          <div
            ...
            :class="{
              'search_box__results--highlight':
                highlightIndex === getHighlightIndex('users', index)
            }"
        ...
        <div
          class="search_box__results__message"
          v-if="!threads.length && !users.length && !loading"
        >
...
export default {
  ...
  methods: {
    setShowResults() {
      ...
      this.showResults =
        this.searchField.trim().length > this.$store.state.MinQueryLength - 1;
      if (this.showResults) {
        this.getResults();
      } else {
    ...
    setKeyHighlight(e) {
      ...
      if (this.highlightIndex === null) {
        ...
        if (updatedIndex === this.totalHighlightOptions || updatedIndex < 0) {
          this.highlightIndex = null;
          return;
        }
