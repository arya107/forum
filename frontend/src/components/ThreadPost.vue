<template>
  ...
  >
    <div
      ...
      :class="{
        'post__quote--show':
          showQuote && allowQuote && showReply && $store.state.username
      }"
      :style="{
        left: quoteX + 'px',
        top: quoteY + 'px'
    ...
    <modal-window v-model="showShareModal" @click.stop="() => {}">
      ...
      <button
        ...
        @click.stop="setShareModalState(false)"
      >
        OK
      </button>
    </modal-window>
    ...
    <div class="post__meta_data">
      <div style="display: inline-flex;">
        ...
        <div class="post__thread" v-if="showThread" @click.stop="goToThread">
          In thread
          <span class="post__thread__name">{{
            post.Thread.name | truncateMid(50)
          }}</span>
          &nbsp;&middot;&nbsp;
        </div>
        <div class="post__user" v-else>
          ...
          <span class="admin_badge" v-if="post.User && post.User.admin"
            >admin</span
          >
        </div>
      ...
      <div class="post__date">
        {{ post.createdAt | formatDate("time|date", ", ") }}
      </div>
    </div>
    <div class="post__date post__date--mobile">
      {{ post.createdAt | formatDate("time|date", ", ") }}
    </div>
    <div
      tabindex="-1"
    ...
    <div class="post__footer">
      <div class="post__footer_group">
        ...
        <div class="post__footer_sub_group" v-if="post.Replies.length">
          <span
            class="post__footer_sub_group__text post__footer_sub_group__text--replies"
            >replies</span
          >
          <post-reply
      ...
      >
        <div
          class="post__action post__share"
          @click.stop="setShareModalState(true)"
        >
          share
        </div>
        <div
...
export default {
  ...
  methods: {
    ...
    goToThread() {
      this.$router.push(
        `/thread/${this.post.Thread.slug}/${this.post.Thread.id}`
      );
    },
    goToPost() {
