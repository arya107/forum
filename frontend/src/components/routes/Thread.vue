<template>
  <div
    ...
    :style="posts.length ? 'padding-bottom: 8.5rem;' : null"
  >
    <confirm-modal
    ...
    <div class="thread_side_bar">
      <loading-button
        ...
        :class="{
          'button--disabled': !$store.state.thread.selectedPosts.length,
        }"
      ...
      <menu-button
        ...
        :options="[
          {
            event: 'lock_thread',
            value: $store.state.thread.locked ? 'Unlock thread' : 'Lock thread',
          },
    ...
    <div class="locked_thread" v-if="$store.state.thread.locked">
      <h2>Thread locked</h2>
      You can't post in this thread because it has been locked by an
      administrator
    </div>
    ...
    <div class="posts">
      <scroll-load @loadNext="loadNextPosts" @loadPrevious="loadPreviousPosts">
...
export default {
  ...
  computed: {
    ...
    editor: {
      get() {
        return this.$store.state.thread.editor.value;
      },
      set(val) {
    ...
    editorState() {
      return this.$store.state.thread.editor.show;
    },
    replyThreadButton() {
  ...
  methods: {
    ...
    setThreadSelectState() {
      this.$store.commit(
        "setShowRemovePostsButton",
        !this.$store.state.thread.showRemovePostsButton
      );
    },
