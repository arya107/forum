<template>
  <div class="route_container">
    <div
      class="forum_description"
      v-if="$store.state.meta.showDescription && $store.state.meta.description"
    >
    ...
    <div class="thread_sorting">
      ...
      <div class="thread_sorting__add_and_categories">
        <select-button
          ...
          :options="categories"
        ></select-button>
        <router-link class="button button--blue" to="/thread/new">
          {{ postNewThreadText }}
        </router-link>
      </div>
    </div>
    <div class="threads_main">
      <div class="threads_main__side_bar">
        <div class="threads_main__side_bar__title">categories</div>
        <router-link
          v-for="(category, $index) in categories"
          :key="'category-link-' + $index"
          class="threads_main__side_bar__menu_item"
          :class="{
            'threads_main__side_bar__menu_item--selected':
      ...
      <transition name="fade" mode="out-in">
        <div class="threads_main__threads" v-if="!threads" key="loading">
        ...
        >
          <template v-if="loadingNewer">
            <thread-display-placeholder
              v-for="n in newThreads"
              :key="'placeholder-upper-' + n"
            >
            </thread-display-placeholder>
          </template>
          <div
            class="threads_main__load_new"
            v-if="newThreads"
            @click="getNewerThreads"
          >
        ...
        <div
          key="no threads"
          v-else
          class="threads_main__threads overlay_message"
        >
...
export default {
  ...
  computed: {
    filteredThreads() {
      ...
      return this.threads
        .filter((thread) => {
          return (
            thread.Category.value === this.selectedCategory ||
  ...
  created() {
    ...
    this.$socket.on("new thread", (data) => {
      if (
        data.value === this.selectedCategory ||
        this.selectedCategory == "ALL"
      ) {
        this.newThreads++;
      }
