<template>
  <div class="admin_moderation">
    ...
    <confirm-modal
      v-model="removePostObj.showConfirmModal"
      @confirm="removePost"
      text="Remove"
      color="red"
    >
    ...
    <confirm-modal
      v-model="removePostObj.showThreadDeleteModal"
    ...
    <transition name="fade" mode="out-in">
      ...
      >
        <div class="admin_moderation__report admin_moderation__report--header">
          <div
            class="admin_moderation__report__post admin_moderation__report--cell_border admin_moderation__report--cell_border-hidden"
          >
            User and post reported
          </div>
          <div
            class="admin_moderation__report__reason admin_moderation__report--cell_border admin_moderation__report--cell_border-hidden"
          >
            Report reason
          </div>
          <div
            class="admin_moderation__report__flagged_by admin_moderation__report--cell_border admin_moderation__report--cell_border-hidden"
          >
            Reported by user
          </div>
          <div class="admin_moderation__report__actions">Actions</div>
        </div>

        <div
          class="admin_moderation__report"
          :key="'report-' + $index"
          v-for="(report, $index) in filteredReports"
        >
          <div
            class="admin_moderation__report__post admin_moderation__report--cell_border"
          >
            <div class="admin_moderation__report__post__header">
              <avatar-icon
                class="admin_moderation__report__flagged_by__avatar"
                size="small"
                :user="report.Post.User"
              ></avatar-icon>
              <div>
                <div class="admin_moderation__report__post__user">
                  {{ report.PostUserUsername }}
                </div>
                <div class="admin_moderation__report__flagged_by__date">
                  Posted {{ report.createdAt | formatDate }}
                </div>
              </div>
            </div>
            <div class="admin_moderation__report__post__content">
              {{ report.Post.content | stripTags | truncate(150) }}
            </div>
          </div>
          <div
            class="admin_moderation__report__reason admin_moderation__report--cell_border"
          >
            {{ report.reason }}
          </div>
          <div
            class="admin_moderation__report__flagged_by admin_moderation__report--cell_border"
          >
            <avatar-icon
              class="admin_moderation__report__flagged_by__avatar"
              size="small"
              :user="report.FlaggedByUser"
            ></avatar-icon>
            <div class="admin_moderation__report__flagged_by__text_info">
              <div class="admin_moderation__report__flagged_by__user">
                {{ report.FlaggedByUserUsername }}
              </div>
              <div class="admin_moderation__report__flagged_by__date">
        ...
        this.axios
          ...
          .then(() => {
            return this.axios.delete(
              "/api/v1/report/" + this.removePostObj.report.id
            );
          })
          .then(() => {
