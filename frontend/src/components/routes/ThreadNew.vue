<template>
  <div class="route_container thread_new">
    ...
    <div class="thread_meta_info">
      <div class="thread_meta_info__text">
        Enter the thread title and the category to post it in
      </div>
      ...
      <transition name="slide">
        <div class="thread_meta_info__poll" v-if="showPoll">
          <div class="thread_meta_info__poll__top_bar">
            ...
            ></fancy-input>
            <button
              class="button button--thin_text button--borderless"
              @click="removePoll"
            >
              Remove poll
            </button>
          ...
          <div>
            ...
            >
              ...
              ></fancy-input>
              <span @click="removePollAnswer($index)" title="Remove answer"
                >&times;</span
              >
            </div>
            <div class="thread_meta_info__form">
              ...
              ></fancy-input>
              <button class="button button--thin_text" @click="addPollAnswer">
                Add answer
              </button>
            </div>
          </div>
    ...
    >
      <div class="editor__input">
        <div class="editor__format_bar editor__format_bar--editor">editor</div>
        <input-editor-core
          v-model="editor"
    ...
    <loading-button
      class="button--green submit"
      :loading="loading"
      @click="postThread"
      >Post thread</loading-button
    >
  </div>
</template>
  ...
  methods: {
    ...
    postThread() {
      ...
      if (this.showPoll && this.pollAnswers.length < 2) {
        errors.push({
          name: "pollAnswer",
          error: "You need at least 2 answers",
        });
      }
      if (
        this.showPoll &&
        this.hasDuplicates(this.pollAnswers, (i) => i.answer)
      ) {
        errors.push({
          name: "pollAnswer",
          error: "Your answers can't contain any duplicates",
        });
      }
      if (errors.length) {
