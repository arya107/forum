  ...
  <div class="route_container">
    ...
    <transition name="fade" mode="out-in">
      <div
        ...
        key="results"
        v-if="threads && threads.length && !loadingThreads"
      >
        <h2>Threads</h2>
        <thread-display
        ...
        <div
          class="search__more search__item"
          v-if="threads.length > $store.state.MinQueryLength - 1"
      ...
      <div key="loading" v-if="loadingThreads">
        <h2>Threads</h2>
    ...
    <transition name="fade" mode="out-in">
      <div
        class="search__results"
        key="results"
        v-if="users && users.length && !loadingUsers"
      >
        <h2>Users</h2>
        <user-display
        ...
        <div
          class="search__item search__more"
          v-if="users.length > 5"
      ...
      <div key="loading" v-if="loadingUsers">
        <h2>Users</h2>
      ...
      >
        ...
        {{ queryTooShort ? "Search term is too short" : "No results found" }}
      </div>
    </transition>
  </div>
</template>
