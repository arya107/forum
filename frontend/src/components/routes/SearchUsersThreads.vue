<template>
  <div class="route_container">
    ...
    <transition name="fade" mode="out-in">
      ...
      >
        <scroll-load :loading="loading" @loadNext="loadNextPage">
      ...
      <div key="loading" v-else>
        <user-placeholder v-if="searchType === 'users'"> </user-placeholder>
...
export default {
  ...
  computed: {
    ...
    showNoResults() {
      return !this.loading && this.results && !this.results.length;
    },
    queryTooShort() {
  ...
  methods: {
    getResults() {
      ...
      this.axios
        .get(
          `/api/v1/search/${this.searchType.slice(0, -1)}?q=${
            this.$route.params.q
          }`
        )
        .then((res) => {
