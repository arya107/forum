<template>
  <div class="heart_button">
    ...
    <font-awesome-icon
      ...
      @click="changeLike"
    />
    <span class="heart_button__count" @click="showModal = true">{{
      likes.length
    }}</span>
  </div>
</template>
...
export default {
  ...
  computed: {
    likeable() {
      let postUsername = this.post.User ? this.post.User.username : null;
    ...
    changeLike() {
      ...
      if (!this.liked) {
        this.axios
          ...
          .then(() => {
            return this.axios.get("/api/v1/user/" + this.$store.state.username);
          })
          .then(res => {
