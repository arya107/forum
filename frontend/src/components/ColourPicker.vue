...
export default {
  ...
  methods: {
    ...
    updatePalettePicker(e) {
      ...
      this.lightness_ = Math.round(
        (100 * (rect.height - centerY)) / rect.height
      );
    },
    updateHuePicker(e) {
      //If the canvas is not loaded
      //Or there's no dragging and not a click event
      if (!this.$refs.hue || (!this.huePicker.dragging && e.type !== "click"))
        return;
