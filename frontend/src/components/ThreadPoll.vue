  ...
  <div class="poll">
    <transition name="slide" mode="out-in">
      ...
      <div key="poll" v-else>
        ...
        <div
          class="poll__voting_view"
          v-if="view === 'voting' && !poll.hasVoted"
        >
        ...
        <div class="poll__results_view" v-else>
          ...
          <div class="poll__results">
            <div
              ...
              v-for="(result, $index) in poll.PollAnswers"
            >
              <div>
                {{ result.answer }}
                <span class="poll__result__info">
                  &middot;
                  {{ result.PollVotes.length }}
                  {{ result.PollVotes.length | pluralize("vote") }} ({{
                    result.percent || 0
                  }}%)
                </span>
              </div>
          ...
          <div class="poll__buttons" v-if="!poll.hasVoted">
            <button class="button button--thin_text" @click="view = 'voting'">
              Back
            </button>
          </div>
        </div>
