<template>
  <div class="admin_new_admin">
    <modal-window v-model="showModal">
      ...
      <button class="button button--modal" slot="footer" @click="toggleModal">
        OK
      </button>
    </modal-window>
    ...
    <div class="category_widget__box">
      <div class="category_widget__text">
        ...
        Click to generate a login link for a new admin account - this will
        expire after 24 hours

        <div v-if="!filteredAdmins">Loading...</div>
        <div v-else>
          <strong v-if="filteredAdmins.length === 0"
            >There are no other admins</strong
          >
          <template v-else>
            ...
            <span
              ...
              :key="admin.username"
            >
              <router-link :to="'/user/' + admin.username">{{
                admin.username
              }}</router-link>
      ...
      <loading-button :loading="loading" @click="getLink"
        >Generate link</loading-button
      >
    </div>
  </div>
</template>
