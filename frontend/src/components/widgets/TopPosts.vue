<template>
  <div class="widgets__top_posts">
    <template v-if="data_.length">
      ...
      >
        <div class="widgets__top_posts__item__number" v-if="thread.Thread">
          {{ $index + 1 }}
        </div>
        <div class="widgets__top_posts__item__info">
          ...
          <div class="widgets__top_posts__item__views" v-if="thread.Thread">
            {{ thread.pageViews }}
            {{ thread.pageViews | pluralize("page view") }}
          </div>
        </div>
      </div>
    </template>
    ...
    <div
      class="widgets__top_posts__overlay widgets__top_posts__overlay--show"
      v-else
    >
    ...
    <div
      class="widgets__top_posts__overlay"
      :class="{ 'widgets__top_posts__overlay--show': loading }"
    >
...
export default {
  ...
  methods: {
    goToThread(thread) {
      if (thread.Thread) {
        this.$router.push(
          "/thread/" + thread.Thread.slug + "/" + thread.Thread.id
        );
      }
    },
  },
