<template>
  <div
    class="widgets__line_chart"
    ref="container"
    :style="{ 'background-color': background }"
  >
    <div
      class="widgets__line_chart__overlay"
      :style="{ 'background-color': background }"
      :class="{ 'widgets__line_chart__overlay--show': loading }"
    >
    ...
    <div
      ...
      v-if="points.length"
    >
      {{ points[tooltipItem].pageViews }}
      {{ points[tooltipItem].pageViews | pluralize(tooltip) }}
    </div>
    <svg>
      <g
        ref="y_axis"
...
export default {
  ...
  data() {
    let x = d3.scaleTime().domain([0, 0]).range([0, 0]);

    let y = d3.scaleLinear().domain([0, 0]).range([0, 0]);
  ...
  methods: {
    setXFunc() {
      let width =
        this.$refs.container.getBoundingClientRect().width - this.padding * 2;
    ...
    setYFunc() {
      let height =
        this.$refs.container.getBoundingClientRect().height - this.padding * 2;
    ...
    updateFuncs() {
      ...
      d3.select(this.$refs.x_axis).call(
        d3.axisBottom(this.x).tickSize(0).ticks(this.points.length)
      );
    },
    showTooltip(e, i) {
