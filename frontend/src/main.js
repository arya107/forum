...
window.onload = () => {
  ...
  div.innerHTML = `<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/highlight.js/9.10.0/styles/default.min.css">`;
...
const SearchUsersThreads = () =>
  import("./components/routes/SearchUsersThreads");
...
const AdminModerationReports = () =>
  import("./components/routes/AdminModerationReports");
const AdminModerationBannedUsers = () =>
  import("./components/routes/AdminModerationBannedUsers");
...
const router = new VueRouter({
  routes: [
    ...
    {
      path: "/thread/:slug/:id/:post_number",
      name: "thread-post",
      component: Thread
    },
    ...
    {
      path: "/search/users/:q",
      component: SearchUsersThreads,
      name: "search/users"
    },
    {
      path: "/search/threads/:q",
      component: SearchUsersThreads,
      name: "search/threads"
    },
    {
...
Vue.filter("truncateMid", function(value, length) {
  if (value.length <= length) {
    ...
    return (
      value.slice(0, firstLen) +
      "…" +
      value.slice(value.length - secondLen, value.length)
    );
  }
});
