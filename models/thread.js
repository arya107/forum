      ...
      instanceMethods: {
        getMeta(limit) {
          ...
          if (!lastPost || lastPost.postNumber + 1 === this.postsCount) {
            ...
            meta.nextURL = `/api/v1/thread/${this.id}?limit=${limit}&from=${
          ...
          if (!firstPost || firstPost.postNumber === 0) {
            ...
            meta.previousURL = `/api/v1/thread/${this.id}?limit=${firstPost.postNumber}&from=0`;
          } else {
            meta.previousURL = `/api/v1/thread/${this.id}?limit=${limit}&from=${
              firstPost.postNumber - limit
            }`;
          }
          ...
          if (lastPost === undefined) {
            ...
            let postsRemaining = this.postsCount - lastPost.postNumber - 1;
