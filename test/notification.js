...
describe('Notifications', () => {
  ...
  describe('/notification', () => {
    it('should return no notifications', (done) => {
      user.get('/api/v1/notification').end((err, res) => {
    ...
    it('should return a mention notification if user mentioned', async () => {
      ...
      res.body.Notifications.should.contain.something.that.has.deep.property(
        'interacted',
        false
      );
      res.body.Notifications.should.contain.something.that.has.deep.property(
        'PostNotification.User.username',
        'useraccount'
      );
      res.body.Notifications.should.contain.something.that.has.deep.property(
        'PostNotification.Post.content',
        '<p>POST 1</p>\n'
      );
  ...
  describe('/notification/:id', () => {
    ...
    it('should return an error if notification does not exist', (done) => {
      admin.put('/api/v1/notification/notanid').end((err, res) => {
        res.should.have.status(400);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.invalidParameter('id', 'invalid notification id')
        );
    ...
    it('should return an error if notification does not exist', (done) => {
      admin.delete('/api/v1/notification/notanid').end((err, res) => {
        res.should.have.status(400);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.invalidParameter('id', 'invalid notification id')
        );
