...
describe('Thread and post', () => {
  ...
  describe('POST /thread', () => {
    ...
    it('should add a slug from the thread name', async () => {
      ...
      res.body.should.have.property(
        'name',
        ' à long thrËad, with lØts of àccents!!!	'
      );
      res.body.should.have.property(
        'slug',
        'a-long-thread-with-lots-of-accents'
      );
      res.body.should.have.deep.property('User.username', 'username');
      res.body.should.have.deep.property('Category.name', 'category_name');
    });
    it('should return an error if not logged in', async () => {
      try {
        ...
        JSON.parse(
          res.response.text
        ).errors.should.contain.something.that.deep.equals(
    ...
    it('should return an error if missing title', async () => {
      try {
        let res = await userAgent.post('/api/v1/thread').send({
          category: 'CATEGORY_NAME',
        });
      } catch (res) {
        ...
        body.errors.should.contain.something.that.has.property(
          'message',
          'name cannot be null'
        );
      }
  ...
  describe('POST /post', () => {
    it('should create a post if logged in', async () => {
      ...
      res.body.should.have.property(
        'content',
        '<p>content <strong>here</strong></p>\n'
      );
    ...
    it('should return an error if mentions are invalid type', (done) => {
      userAgent
        ...
        .end((err, res) => {
          ...
          res.body.errors.should.contain.something.that.has.property(
            'message',
            'mentions must be an array of strings'
          );
    ...
    it('should return an error if reply id does not exist', async () => {
      try {
        ...
        res.should.have.status(400);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.invalidParameter('replyingToId', 'post does not exist')
        );
      } catch (res) {
        let body = JSON.parse(res.response.text);

        res.should.have.status(400);
        body.errors.should.contain.something.that.deep.equals(
          Errors.invalidParameter('replyingToId', 'post does not exist')
        );
      }
    });
  ...
  describe('GET /thread/:id', () => {
    it('should return the thread and corresponding posts', async () => {
      ...
      res.body.Posts.should.contain.something.that.has.property(
        'content',
        '<p>content</p>\n'
      );
      res.body.Posts.should.contain.something.that.has.deep.property(
        'User.username',
        'username'
      );
    ...
    it('should allow pagination', async () => {
      ...
      for (var i = 0; i < 30; i++) {
        ...
        if (i === 3) {
          await userAgent
            ...
            .set('content-type', 'application/json')
            .send({
              threadId: threadOther.body.id,
              content: `POST OTHER ${i}`,
            });
        }
      }
      ...
      let pageInvalid = await userAgent.get(
        '/api/v1/thread/' + thread.body.id + '?from=' + 100
      );
      ...
      pageThree.body.Posts[0].should.have.property(
        'content',
        '<p>POST 20</p>\n'
      );
      pageThree.body.Posts[9].should.have.property(
        'content',
        '<p>POST 29</p>\n'
      );
    ...
    it('should allow you to get an individual and surrounding posts', async () => {
      ...
      let pageOne = await http.get(
        `/api/v1/thread/${PAGINATION_THREAD_ID}?postNumber=15`
      );
      ...
      pageZero.body.Posts[9].should.have.property(
        'content',
        '<p>POST 10</p>\n'
      );
      ...
      pageFirst.body.Posts[0].should.have.property(
        'content',
        '<p>POST 0</p>\n'
      );
    ...
    it('should return an error if :id is invalid', async () => {
      try {
        ...
        res.should.have.status(400);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.invalidParameter('id', 'thread does not exist')
        );
      } catch (res) {
        let body = JSON.parse(res.response.text);
  ...
  describe('DELETE /post/:id', () => {
    ...
    it('should remove the post', async () => {
      ...
      post.body.should.have.property(
        'content',
        '<p>[This post has been removed by an administrator]</p>\n'
      );
    });
    ...
    it('should return an error if post does not exist', (done) => {
      userAgent.delete('/api/v1/post/not_a_post').end((err, res) => {
        res.should.be.json;
        res.should.have.status(400);
        res.body.errors.should.include.something.that.has.property(
          'message',
          'post does not exist'
        );
    ...
    it('should return an error if not an admin', (done) => {
      normalUserAgent.delete('/api/v1/post/' + postId).end((err, res) => {
        res.should.be.json;
        res.should.have.status(401);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.requestNotAuthorized
