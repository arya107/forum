...
describe('Report', () => {
  ...
  describe('POST /report', () => {
    ...
    it('should be fine with multiple reports from one user', async () => {
      ...
      report1.should.have.deep.property(
        'FlaggedByUser.username',
        'useraccount'
      );
      ...
      report2.should.have.deep.property(
        'FlaggedByUser.username',
        'useraccount'
      );
    ...
    it('should return an error if invalid report reason provided', (done) => {
      userAccount
        ...
        .end((err, res) => {
          ...
          res.body.errors.should.contain.something.that.has.property(
            'message',
            'Report reason can only be one of the pre-defined options'
          );
  ...
  describe('GET /report', () => {
    ...
    it('should return all reports', async () => {
      let res = await adminAccount.get('/api/v1/report');

      res.should.have.status(200);
      res.should.be.json;

      res.body.should.have.length(2);
      res.body.should.contain.something.with.deep.property(
        'FlaggedByUser.username',
        'useraccount'
      );
    ...
    it('should return an error if not admin account', (done) => {
      userAccount.get('/api/v1/report').end((err, res) => {
        res.should.have.status(401);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.requestNotAuthorized
        );
  ...
  describe('DELETE /report/:id', () => {
    ...
    it('should return an error if not an admin', (done) => {
      userAccount.delete('/api/v1/report/2').end((err, res) => {
        res.should.have.status(401);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.requestNotAuthorized
        );
    ...
    it('should return an error if invalid id', (done) => {
      adminAccount.delete('/api/v1/report/fake').end((err, res) => {
        res.should.have.status(400);
        res.body.errors.should.contain.something.that.has.property(
          'message',
          'Post id is not valid'
