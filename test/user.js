...
describe('User', () => {
  ...
  describe('/ POST user', () => {
    ...
    it('should create an admin account provided with a token', async () => {
      ...
      try {
        let invalidAccountRes = await chai
          .request(server)
          .post('/api/v1/user')
          .set('content-type', 'application/json')
          .send({
        ...
        invalidAccountRes.body.errors.should.include.something.that.deep.equals(
          Errors.invalidToken
        );
      } catch (res) {
        res.should.have.status(401);
        JSON.parse(
          res.response.text
        ).errors.should.include.something.that.deep.equals(Errors.invalidToken);
      }
    });
  ...
  describe('/:username GET user', () => {
    it('should return the user', async () => {
      let res = await chai.request(server).get('/api/v1/user/username');
    ...
    it('should return an error if username invalid', async () => {
      try {
        let res = await chai.request(server).get('/api/v1/user/not_a_user');
    ...
    it('should get posts as well if posts query is appended', async () => {
      ...
      let res = await agent.get('/api/v1/user/adminaccount?posts=true');
      ...
      res.body.Posts[0].should.have.deep.property(
        'User.username',
        'adminaccount'
      );
    });
    ...
    it('should allow pagination', async () => {
      ...
      let pageOne = await agent.get(
        '/api/v1/user/paginationaccount?posts=true'
      );
  ...
  describe('/:username/logout POST user', () => {
    ...
    it('should log out the user', (done) => {
      agent
        ...
        .end((err, res) => {
          agent.post('/api/v1/user/username/logout').end((err, res) => {
            res.should.have.status(200);
            res.should.not.have.cookie('username');
  ...
  describe('/:username DELETE', () => {
    ...
    it('should return an error if username is not logged in', (done) => {
      agent.delete('/api/v1/user/notloggedin').end((err, res) => {
        res.should.have.status(401);
        res.body.errors.should.contain.something.that.deep.equals(
          Errors.requestNotAuthorized
        );
